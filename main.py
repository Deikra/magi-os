import flet as ft
from flet_matplotlib import MatplotlibChart
import matplotlib
...
def main(page: ft.Page):
    page.title = "MAGI OS 4.1"
    page.theme_mode = ft.ThemeMode.DARK
    page.bgcolor = BG_COLOR
    
    # Configuramos la ventana de Windows/Mac para que tenga tamaño de celular
    page.window.width = 400
    page.window.height = 800
    page.window.always_on_top = True # Opcional: la mantiene siempre visible
    ...
    def view_estado():
        ...
        grafica_container = ft.Container(height=200)
        ...
        def actualizar_datos_estado(e=None):
            ...
            vals, cols, labs = [], [], []
            if hipo > 0: vals.append(hipo); cols.append(DANGER_RED); labs.append("Hipo")
            if optimo > 0: vals.append(optimo); cols.append(NEON_GREEN); labs.append("Óptimo")
            if hiper > 0: vals.append(hiper); cols.append(WARNING_ORANGE); labs.append("Hiper")
            
            if vals:
                ax.pie(vals, labels=labs, colors=cols, autopct='%1.1f%%', textprops={'color': TEXT_WHITE})
                grafica_container.content = MatplotlibChart(fig, expand=True, transparent=True)
            else:
                grafica_container.content = ft.Text("Sin datos", color=TEXT_MUTED)
        ...
        return ft.Column([
            ...
            dd_filtro,
            ft.Container(content=grafica_container, bgcolor=CARD_BG, border_radius=10, padding=10),
    ...
    def view_energia():
        ...
        tabla = ft.DataTable(
            columns=[ft.DataColumn(ft.Text("Alimento")), ft.DataColumn(ft.Text("Cat.")), ft.DataColumn(ft.Text("C/K", tooltip="Carbs/Kcal"))],
            rows=[]
        )
        ...
        def enseñar(e):
            mostrar_alerta("Opción disponible pronto en V4.0", NEON_PURPLE)
    ...
    app_bar = ft.AppBar(
        ...
        title=ft.Text("MAGI OS 4.1", color=TEXT_WHITE, font_family="Courier"),
        bgcolor=CARD_BG,
    )
...
# Aquí ejecutamos como App Nativa de Escritorio
ft.app(target=main)